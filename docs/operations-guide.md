...
- as of 2026-04-07, `ops:status` reports `alarmSubscriptionReadiness.ready = true` and `allClear = true` in both staging and production after the chosen inbox confirmed the SNS subscriptions
...
- the chosen alert inbox is now confirmed in both staging and production
- `alarmSubscriptionReadiness.confirmedCount >= 1`
- `allClear = true`
- the remaining alerting improvements are optional routing enhancements, not a readiness requirement
